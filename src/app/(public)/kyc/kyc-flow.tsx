...
export function KycFlow({ initialEmail, initialName, orderNumber }: KycFlowProps) {
  const hasInitialData = !!(initialEmail && initialName)
  const [step, setStep] = useState<KycStep>(hasInitialData ? 'id_type' : 'start')
  ...
  return (
    <div>
      ...
      {step === 'id_capture_thickness' && (
        <CameraCapture
          title="身分証明書の厚みを撮影"
          description="カードを斜めに傾けて、厚みがわかるように撮影してください"
          guideType="rectangle"
          facingMode="environment"
      ...
      {step === 'face_capture' && (
        <CameraCapture
          ...
          onBack={() => {
            if (needsBackImage) setStep('id_capture_back')
            else if (needsThicknessImage) setStep('id_capture_thickness')
            else setStep('id_capture_front')
          }}
        />
      )}
