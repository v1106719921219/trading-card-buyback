...
type Row=Ref&{key:string;name:string}
type Product={id:string;name:string;model_number:string|null;set_number:string|null;price:number}
...
export default function ComparisonPage(){
 ...
 useEffect(()=>{void load()},[])
 const rows=useMemo(()=>data?[...data.products.map(p=>({key:`product-${p.id}`,name:p.name,productId:p.id,quotes:empty()})),...data.candidates].map(r=>({...r,...data.saved[r.key]})):[],[data])
